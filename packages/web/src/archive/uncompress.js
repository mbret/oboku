...
(function () {
  ...
  async function archiveOpenFile(file, password) {
    ...
    const archive = await archiveOpenArrayBuffer(file_name, password, array_buffer, file);
  ...
  function _rarGetEntries(rar_handle) {
    // Get the entries
    let info = window.readRARFileNames(rar_handle.rar_files, rar_handle.password);
    let entries = [];
