...
export function isRarFile(array_buffer) {
...
;(function () {
  ...
  function loadArchiveFormats(formats, cb) {
    ...
    window.g_on_loaded_cb = checkForLoadDone
    ...
    formats.forEach(function (archive_format) {
      ...
      switch (archive_format) {
        case "rar":
          unrarMemoryFileLocation = "/libunrar.js.mem"
          window.unrarMemoryFileLocation = "/libunrar.js.mem"
    ...
    // just run cb if there is nothing to load
    if (!loading) return cb()
  }

  async function archiveOpenFile(file, password) {
    // Get the file's info
    let blob = file.slice()
    ...
    const archive = await archiveOpenArrayBuffer(
      file_name,
      password,
      array_buffer,
      file
  ...
  async function archiveOpenArrayBuffer(
    file_name,
    password,
    array_buffer,
    data
  ...
  function _rarGetEntries(rar_handle) {
    // Get the entries
    let info = window.readRARFileNames(
