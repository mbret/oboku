            ...
            if (yes && database) {
                try {
                    await Promise.all(data.map(async ([resourceId]) => {
                        ...
                        const booksToReattachToMergedLink = dataAsJson.map(data => data.book).filter(id => !!id) as string[]
                        ...
                        const bookDocs = await database.book.find({ selector: { _id: { $in: booksToReattachToMergedLink } } }).exec()

                        await Promise.all(bookDocs.map(async doc => {
