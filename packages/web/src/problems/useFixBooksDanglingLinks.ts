...
export const useFixBooksDanglingLinks = () => {
    ...
    const removeDanglingLinksFromBook = useCallback(async (doc: BookDocument) => {
        if (doc.links.length === 0) return

        const existingLinksForThisBook = await db?.link.safeFind({
            selector: {
                _id: {
                    $in: doc.links
                }
        ...
        const toRemove = difference(doc.links, existingLinksForThisBook?.map(doc => doc._id) ?? [])
