...
const resolvePagesForGallery = (doc: Document) => {
  ...
  return Array.from(
    doc.querySelectorAll(
      `#content #thumbnail-container .thumb-container a img.lazyload`
    )
  ).map((element) => (element as HTMLImageElement).dataset.src || ``)
}

const resolvePagesGalleryRealId = (doc: Document) => {
  const firstImgElementForFirstThumbnailElement = doc.querySelector(
    `#content #thumbnail-container .thumb-container a img`
  )
  ...
  const removeUnwantedPart = firstImgElementForFirstThumbnailLink.substring(
    firstImgElementForFirstThumbnailLink.lastIndexOf(knownPrefix) +
      knownPrefix.length
  )
