...
import { atom, selectorFamily, useRecoilCallback, useRecoilState, useRecoilValue, useSetRecoilState } from "recoil"
...
const retryFetch = async (resource: string, retryNumber = 0): Promise<Blob> => {
  ...
  try {
    ...
    return await new Promise<Blob>((resolve, reject) => {
      setTimeout(() => {
        retryFetch(resource, retryNumber + 1).then(resolve).catch(reject)
      }, 500)
    })
  }
}
  ...
  return useRecoilCallback(({ snapshot, set }) => async (asset: Asset) => {
    ...
    try {
      if (asset.missingHigherOrder) {
        let data = await localforage.getItem<({ blob: Blob, order: number }) | null>(asset.id)
        if (typeof data?.order === 'number') {
          startFromIndexOrder = data.order + 1
        }
      }

      await Promise.all(asset.resources.map(async (resource, indexOrder) => {
        if (indexOrder < startFromIndexOrder) return
        const blob = await retryFetch(resource)
        const data = { blob: blob, order: indexOrder }
        const existingData = await localforage.getItem<{ blob: FileType, order: number }>(asset.id)
        if (data.order >= (existingData?.order || 0)) {
          try {
            ...
            if ((e as any)?.name === 'DataCloneError' && (e as any)?.code === 25) {
          ...
          set(assetsState, old => old.map(item => item.id === asset.id
            ? {
              ...item,
      ...
      set(assetsState,
        old => old.map(item => item.id === asset.id
          ? {
            ...item,
            downloading: false,
...
export const CachedResourcesProvider: FC = memo(({ children }) => {
   ...
   */
  useEffect(() => {
    const assetsToDownload = assets
      .filter(asset => (asset.state === 'missing' || asset.missingHigherOrder) && !asset.downloading && asset.state !== 'failed')
