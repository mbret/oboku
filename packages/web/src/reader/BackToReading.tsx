...
export const BackToReading = () => {
  const [bookBeingRead, setBookBeingReadState] = useRecoilState(bookBeingReadState)
  const dialog = useDialogManager()
  const history = useHistory();

  useEffect(() => {
    if (!bookBeingRead || history.location.pathname.startsWith(BASE_READER_ROUTE)) return
