...
export const normalizedBookDownloadsState = atom<Record<string, {
...
export const bookDownloadsSizeState = selector({
  ...
  get: ({ get }) => {
    ...
    return books.filter(book => book?.downloadState === DownloadState.Downloaded).reduce((size, item) => size + (item?.size || 0), 0)
  }
})